...
InitStatus R3BOnlineSpectra::Init()
{
    ...
    for (Int_t ifibcount = 0; ifibcount < 14; ifibcount++)
    {
        ...
        std::stringstream histName1, histName2, histName3, histName4, histName5, histName6, histName7, histName8,
            histName9, histName10, histName11, histName12;
        std::stringstream histTitle1, histTitle2, histTitle3, histTitle4, histTitle5, histTitle6, histTitle7,
...
void R3BOnlineSpectra::Exec(Option_t* option)
{
            ...
            /*
                ...
                //if(nPart > 1){
                 cout<<fNEvents<<", "<<Multip<<", "<<nPart<<", "<<iPart<<", VFTX: "<<
                                    calData->fTimeV_lt_ns<<", "<<calData->fTimeV_l_ns<<",
               "<<calData->fTimeV_lb_ns<<",
               "<<calData->fTimeV_b_ns<<", "<<
                                    calData->fTimeV_rb_ns<<", "<<calData->fTimeV_r_ns<<",
               "<<calData->fTimeV_rt_ns<<",
               "<<calData->fTimeV_t_ns<<endl;
                 cout<<fNEvents<<", "<<Multip<<", "<<nPart<<", "<<iPart<<", TLEAD: "<<
                                    calData->fTimeL_lt_ns<<", "<<calData->fTimeL_l_ns<<",
               "<<calData->fTimeL_lb_ns<<",
               "<<calData->fTimeL_b_ns<<", "<<
                                    calData->fTimeL_rb_ns<<", "<<calData->fTimeL_r_ns<<",
               "<<calData->fTimeL_rt_ns<<",
               "<<calData->fTimeL_t_ns<<endl;
                 cout<<fNEvents<<", "<<Multip<<", "<<nPart<<", "<<iPart<<", TTRAIL: "<<
                                    calData->fTimeT_lt_ns<<", "<<calData->fTimeT_l_ns<<",
               "<<calData->fTimeT_lb_ns<<",
               "<<calData->fTimeT_b_ns<<", "<<
                                    calData->fTimeT_rb_ns<<", "<<calData->fTimeT_r_ns<<",
               "<<calData->fTimeT_rt_ns<<",
               "<<calData->fTimeT_t_ns<<endl;
            ...
            */
            ...
            if (iDet == 1)
            {
                ...
                if (1 == 1)
                {
                    ...
                    //        cout<<"R3BOnline WARNING!! LOS VFTX time < 0 or nan! If nan we take TAMEX time, if <0
                    //        no
    ...
    if (fMappedItemsTofd)
    {
        ...
        for (Int_t ihit = 0; ihit < nHits; ihit++)
        {
            ...
            //  cout<<ihit<<", "<<iPlane<<"; "<<iPlaneMem<<"; "<<iBar<<"; "<<iBarMem<<",
    ...
    if (fCalItemsTofd)
    {
        ...
        for (Int_t ihit = 0; ihit < nHits; ihit++)
        {
            ...
            if (iBar != iBarMem ||
                NumPaddles[iPlane - 1] ==
                    1) // condition on NumPaddles to be removed when more paddles in plane 3 and 4 activ!!!
            {
                jmult[iPlane - 1][iBar - 1] = 0;
                if (!(IS_NAN(hit->fTime1L_ns)))
                    t1l[0][iPlane - 1][iBar - 1] = hit->fTime1L_ns;
                if (!(IS_NAN(hit->fTime1T_ns)))
                    t1t[0][iPlane - 1][iBar - 1] = hit->fTime1T_ns;
                if (!(IS_NAN(hit->fTime2L_ns)))
                    t2l[0][iPlane - 1][iBar - 1] = hit->fTime2L_ns;
                if (!(IS_NAN(hit->fTime2T_ns)))
                    t2t[0][iPlane - 1][iBar - 1] = hit->fTime2T_ns;
            ...
            else
            {
                ...
                if (!(IS_NAN(hit->fTime1L_ns)))
                    t1l[jm][iPlane - 1][iBar - 1] = hit->fTime1L_ns;
                if (!(IS_NAN(hit->fTime1T_ns)))
                    t1t[jm][iPlane - 1][iBar - 1] = hit->fTime1T_ns;
                if (!(IS_NAN(hit->fTime2L_ns)))
                    t2l[jm][iPlane - 1][iBar - 1] = hit->fTime2L_ns;
                if (!(IS_NAN(hit->fTime2T_ns)))
                    t2t[jm][iPlane - 1][iBar - 1] = hit->fTime2T_ns;
                ...
                for (Int_t jm = 0; jm < jmult[ipl][ibr] + 1; jm++)
                {
                    ...
                    if (Bar_present[jm][ipl][ibr])
                    {
                        ...
                        if (!(IS_NAN(timeLos[ilc])) && timeLos[ilc] > 0.)
                        {
                            ...
                            if (ipl > 0)
                            {
                                fh_tofd_dt[ipl - 1]->Fill(
                                    iBar, t_paddle[jm][ipl][iBar - 1] - t_paddle[jm][ipl - 1][iBar - 1]);
                            }
